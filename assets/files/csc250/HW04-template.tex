...
\collaborators}
\rhead{CSC250 Spring 2026 - Assignment 4\\
\today{}}
\cfoot{p. \thepage}
...
$ZerosAndMoreOnes = \{w \in \{0,1\}^* \; \vert \; w = 0^i1^j, i < j\}$ is not regular.
...
% --------------
\begin{exercise}{3}
Give context-free grammars for each of the following languages.\begin{enumerate}[(a)]
	\item $\Sigma = \{0,1\}$ and $\{w \ | \ w \texttt{ starts and ends with a different symbol }\}$
	% -------------------------------------------
	...
	\item $\Sigma = \{0,1\}$ and $\{w \ | \texttt{ the length of } \ w \texttt{ is odd }\}$
	% -------------------------------------------
	...
	\item $\Sigma = \{a,b, c\}$ and $\{w = a^ib^jc^k \ | i, j, k \geq 0 \texttt{ and } i+j = k \}$
	% -------------------------------------------
