...
\begin{document}
\newcommand{\duedate}{09:25AM EST Monday March 9th, 2026}
...
\vspace{1em}
This is the first of two examinations for\\ \textbf{CSC250: Theory of Computation}\\ as taught by Pablo Frank Bolton in Spring 2026.\\
\vspace{1em}
\begin{itemize}
	\item your own notes
	\item lecture slides / videos
	\item scribe notes (if you had a scribe assigned to you)
...
\begin{itemize}
    \item There are a \textbf{total of 50 points}. There is a bonus question at the end  worth 2 additional points.
...
\begin{enumerate}[(a)]
	\item \textit{(2 points) In Theory of Computation, how do we analyze problems and solutions for a large variety of problems (how do we make them ready for a computer)?}
  ...
  \;\\
	...
	\item \textit{(2 points) In the Language categories map (the diagram with concentric ovals, each with a different language group), what differentiates each category of languages (Finite, Regular, Context-Free, etc)?}
    ...
    \end{solution}
        ...
        \item \textit{(2 points) What is the idea behind using the Pumping Lemma to prove a language is not Regular?}
...
\begin{eqnarray*}
	r & = & \texttt{It is raining.}\\
	b & = & \texttt{We want to go the beach.}\\
	s & = & \texttt{We put on sunscreen.}
\end{eqnarray*}
...
\[L = \{w \ | \ w \texttt{ has an odd length and must contain the exact substring 000}\}\]
...
\begin{solution}
    ...
    FALSE. \\
    $L1 = L(0^*) \quad \text{ and }  L2 = L(0^*1^*),  \text{ which are regular, } \\
    \text{ is simply the set of words that have only zeros, or } L(0^*), \text{ which is infinite and regular.} $
\end{solution}
...
\begin{solution}
    ...
    TRUE. If L is regular, then it has an FA. The complement of L, or $L^C$ can be obtained by switching accept and reject states. This means $L^C$ has a FA and is therefore regular.
\end{solution}

\item A finite automaton with more than one state cant accept the empty string $\varepsilon$.
\begin{solution}
   \;

   FALSE. drawn on the board.
\end{solution}
...
Prove that the language: \[L_{aba} = \{w \ | \ w = 1^a0^b \; ; \quad a = 3b ; \; a, b \geq 0 \} \] is \textbf{NOT} a regular language.

Note, example words in this language include

$\epsilon, 1110, 11111100,...$
\end{question}
...
\begin{itemize}
    \item We'll assume $L_{aba}$ is regular
    \item Therefore, it must have a finite automaton. And since the language is infinite, that automaton must have a loop.
    \item We can pick the word $w = 1^N01^{N}$ with $N$ being the pumping length.
    \item we can express $w$ in the following way: $w = xyz$ such that:\\
    \begin{align*}
        &x = 1^p \qquad \text{ since } \quad |xy| \leq N \\
        &y = 1^q \qquad \text{ since } \quad |xy| \leq N  \\
        &z = 1^r01^{N} \qquad \text{ the leftover symbols in } w \\
        &\text{with } p+q+r = N
    \end{align*}
    \item If we pump $w$ into $w^1 = xyyz$ we obtain \\$w^1 = 1^p1^q1^q1^r01^{N}$ and since $q>0$ then \\
    $p+q+q+r + 1 \geq N$\\
    which means $w^1 \notin L_{aba}$
    \item Which is a contradiction
    \item  And therefore, $L_{aba}$ is NOT regular
...
Prove that the language: \[L_{aba} = \{w \ | \ w = 1^a0^b \; ; \quad a = 3b ; \; a, b \geq 0 \} \] is Context Free.
...
\begin{align*}
    ...
    R &= \{ S \rightarrow_g ASB \;\vert\; \epsilon \\
      & \qquad A \rightarrow_g A1 \;\vert\; 1   \\
      & \qquad B \rightarrow_g 00B \;\vert\; 00\}
\end{align*}
...
\clearpage
\begin{question}{BONUS}\textbf{(4 points)} \\\\

Prove that a Finite Automaton with N states, that accepts an infinite number of words, must have a loop.
